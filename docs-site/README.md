...
[Astro](https://astro.build/) + [Starlight](https://starlight.astro.build/), configured to serve the repository's real `../docs/` folder directly (see `src/content.config.ts`'s comment) - the content here is not a copy, it's the same ADRs and design docs every source comment in the codebase already links to by that same path. Each top-level entry under `docs/` is symlinked individually into the content collection. The original planning material (a PRD and a phase-by-phase build plan) is not part of this repository; the requirement IDs, acceptance criteria, and milestones that source comments cite are indexed publicly in `docs/design/requirements.md`. Deployed to GitHub Pages by `.github/workflows/docs.yml` on every push to `trunk` that touches `docs/` or `docs-site/`.
