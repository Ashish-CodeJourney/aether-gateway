...
[Docusaurus](https://docusaurus.io/), configured to serve the repository's real `../docs/` folder directly (see `docusaurus.config.ts`'s `path` comment) - the content here is not a copy, it's the same PRD, ADRs, and design docs every source comment in the codebase already links to by that same path. `docs/plan/` (the phase-by-phase build plan and STATUS.md) is deliberately excluded (see the `docs.exclude` comment in `docusaurus.config.ts`) - it's gitignored at the repo root as internal planning material, not part of the public portfolio. Deployed to GitHub Pages by `.github/workflows/docs.yml` on every push to `trunk` that touches `docs/` or `docs-site/`.
